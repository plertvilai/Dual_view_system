function [all_tracks,analysis_time] = videoParticleTracking(vidname,config, stereoParams)
% perform particle tracking from a video
% INPUT:
...
%       - detection_params = parameters for particle detection
%               [global thresh, min blob size, max blob size in pixels]
...
for k=1:1:size(frames,4)
    ...
    mask1 = im1>config.detection_params(1);
    mask1 = bwareafilt(mask1,config.detection_params(2:3));
    
    mask2 = im2>config.detection_params(1);
    mask2 = bwareafilt(mask2,config.detection_params(2:3));
